...
const ArticleViewer: React.FC<ArticleViewerType> = ({ isLight, contentStr }) => {
  ...
  return (
    <div className={styles['markdown-style']}>
      {showTocPin && (
        <span
          className="hidden lg:inline-block toc-anchor fixed top-16 hover:rotate-45 transition-all duration-TRANSITION_DURATION opacity-70 hover:scale-110 hover:opacity-100 left-16 text-lg xl:text-2xl cursor-pointer"
          onClick={handleResetTocPosition}
        >
          <LocateMenu />
        </span>
      )}
            ...
            [
              rehypeAutolinkHeadings,
              {
                ...
                content: {
                  type: 'element',
                  tagName: 'span',
                  properties: { className: 'autolink-icon opacity-0 hover:opacity-100' },
              ...
              useEffect(() => {
                ...
                if (navRef && navRef.current) {
                  ...
                  setDragConstraints({
                    left: 0,
                    top: 0,
                    right: innerWidth - offsetWidth - 2 * gap,
                    bottom: innerHeight - offsetHeight - 2 * gap,
                  });
                  // animationControls.start({
                  //   opacity: 1,
                  // });
